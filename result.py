...
gl_model = load(os.path.join(MODEL_DIR,'RandomForest_ModelGl.joblib'))  # Assuming the same model for both hb and gl

# Load the dataset
df_ca = pd.read_csv(os.path.join(DATA_DIR,"interpolatedca.csv"))
df_hb = pd.read_csv(os.path.join(DATA_DIR,"interpolatedHb.csv"))
df_gl = pd.read_csv(os.path.join(DATA_DIR,"interpolatedgl.csv"))  # Assuming the same dataset for hb and gl
